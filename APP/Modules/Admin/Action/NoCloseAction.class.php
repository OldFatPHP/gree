<?php 
	...
	*2.editHandle，处理编辑请求
	*3.export,导出excel
	*4.NoCloseHandle,处理关闭请求
	*/
	class NoCloseAction extends CommonAction{
...
*/
		...
		public function NoCloseHandle(){			
			if(!$this->isPost()){
				$this->success('请登录',U('Login/Index'));die();
			}
			//dump (I('installIDs'));
			$IDs = I('installID');
			$update['installStatus'] = 7;
			$update['installEndTime'] = date('Y-m-d H:i:s');
			for($i=0;$i<count($IDs);$i++){
				
				$where['installID'] = $IDs[$i] ;
				$data = M('install_information')->where($where)->save($update);
				if(!$data){
					$this->error('关闭失败，请稍后重试！',U('index'));die();
				}
