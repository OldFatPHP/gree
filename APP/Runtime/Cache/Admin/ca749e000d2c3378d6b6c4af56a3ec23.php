...
<body>
       <form  name="form1" action="<?php echo U('add');?>"  method="post"> 

       <table class='table'>
       		<tr>
			<th align='right'>小类查询</th>
			<td width='90%'>
		...
		<table class='table'>
		   ...
		   <tr>
		     ...
		     <td style="text-align:center;"> <?php if(is_array($array)): $i = 0; $__LIST__ = array_slice($array,1,1,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; echo ($vo["name"]); endforeach; endif; else: echo "" ;endif; ?></td>   
		       <?php if(is_array($array)): $i = 0; $__LIST__ = array_slice($array,2,1,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input type="hidden" name="id" value="vo['id']"/>
		       <td style="text-align:center;"> 
				<a href="<?php echo U('CategoriesModify/update',array('id' => $vo['id']));?>" >修改</a>
